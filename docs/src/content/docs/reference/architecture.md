...
| `src/persist.rs` | UI preferences saved to `~/.config/polygit/state.json` (columns, sort, icon style, padding, theme, background, contrast, help tab, pane-splitter mode, grouping, collapsed groups, tree toggle, collapsed folders, repo-page columns, repo-page info panel, repo-page maximize). Per-field tolerant deserializers absorb removed enum values (old `sort_column: "discovery"` → `Name`) and a missing/corrupt file loads from `{}` so field defaults apply. |
...
| `src/explorer.rs` | The two-pane file explorer (`Ctrl+E`). Its floating window is a `tuilith::float::Window` — a corner, an inset and a size, resolved each frame rather than stored — so position and size persist and survive a terminal resize. |
