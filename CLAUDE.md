...
3. **Hover highlight** — a matching branch in **`apply_hover` (`render/mod.rs`)** that pushes the region into `button_hits` (buttons) or `hits` (rows). `apply_hover` is per-modal: it ONLY inspects the foreground modal's own click vecs, so a new button in an existing modal needs its region added to that modal's `else if app.show_<modal>` branch. **A registered click region with no `apply_hover` branch = a button that's clickable but dead on hover** — exactly the recurring miss. Reuse `modal_window_buttons`/`modal_close_button`/`footer_chip`/the `hint_click` footer machinery where possible (they're already hover-wired); only hand-rolled in-body buttons (e.g. the picker's `[pin]`) need a fresh `apply_hover` line. After adding any clickable, grep `apply_hover` for the modal's branch and confirm the new region is handled.
