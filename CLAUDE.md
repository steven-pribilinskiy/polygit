...
`polygit` is an interactive multi-repo `git pull` dashboard: a Rust/ratatui TUI that recursively discovers every git repo under a directory and pulls them in parallel with live per-repo logs. Repos can be viewed flat, grouped (`groups.json`), or as a collapsible directory tree (or tree+groups). The Rust build is the only implementation (an old bash original, `polygit-repos`, was removed once `src/plain.rs` superseded it — it lives on in git history).
...
- **Prepend a `CHANGELOG.md` entry for every version bump** — `## vX.Y.Z — YYYY-MM-DD` then the notes (mirror the commit subject + key bullets). The app embeds `CHANGELOG.md` via `include_str!` for its in-app Changelog / What's New modals, and `make build` runs *before* the release commit, so the entry must exist in the file at build time (it can't be derived from the not-yet-made git commit).
...
- **`git.rs`** — every `git` subprocess call + output classification/parsing + the recursive repo walker. `classify_pull_output` maps stdout/stderr+exit to a `PullOutcome` (incl. throttle detection). `spawn_repo_walker`/`should_descend`/`discover_repos_recursive` do the pruned recursive scan; `relative_path` renders a repo's path relative to the scan root. **Base-branch resolution** order is `resolve_base` = explicit override (no git calls) → `detect_base_branch` → `default_base_branch`. `detect_base_branch` weighs local + remote candidates and picks the fork parent via the pure `choose_base` (smallest commits-unique-to-the-branch; ties → conventional-name then remote then alphabetical), so a branch cut from a non-`main` base resolves correctly.
- **`plain.rs`** — the `--no-tui` path. Output is **byte-compatible with the original bash `polygit-repos` script (now removed; in git history) for a single-level scan** (`--depth 1` / a flat dir); a recursive scan additionally lists nested repos by their relative path. Grouping and the tree are TUI-only; plain mode does **not** do throttle adaptation/auto-retry (it's a one-shot batch).
...
- **`pr_cache.rs`** — persisted PR cache (`pr-cache.json`): `"{repo_abs_path}\u{1f}{branch}"` → `{pr, checked_at}`, 5-min TTL (`is_fresh`). `pr: None` (no open PR) is a cached result. `AppState.pr_cache` is consulted before any `gh` lookup (selected repo via `maybe_resolve_pr`; the all-repos `run_all_prs` batch via a snapshot) and upserted from resolved repos by `flush_pr_cache` on settle/quit.
...
1. **Click region** captured in render + hit-tested in `main.rs` (the capture-then-hit-test above).
2. **Key binding** that does the same thing (keyboard-first rule), surfaced in the element (footer-hint style).
3. **Hover highlight** — a matching branch in **`apply_hover` (`render/mod.rs`)** that pushes the region into `button_hits` (buttons) or `hits` (rows). For a **modal**, `apply_hover` inspects only the foreground modal's own click vecs (its `else if app.show_<modal>` branch). The **non-modal** case (the panes) is different: hover follows the **cursor across whatever panes rendered this frame** — it is *not* gated on focus, and the docked repo page no longer suppresses the list/info/result hovers. Each pane's regions are gated by visibility (`repo_visible`/`list_visible`/`right_visible`, derived from `app.maximized`) so a maximized pane's stale neighbours can't false-match. **A registered click region with no `apply_hover` branch = a button that's clickable but dead on hover** — exactly the recurring miss. Reuse `modal_window_buttons`/`modal_close_button`/`footer_chip`/the `hint_click` footer machinery where possible (they're already hover-wired); pane maximize buttons go through `max_click` (already hover-wired). After adding any clickable, grep `apply_hover` and confirm the new region is handled.

### Pane maximize (`maximized: Option<Pane>`)

Every pane ([1] list · [2] info · [3] result · [4] repo page) is maximizeable. `app.maximized` is the single source of truth; `m` toggles the **active** pane (`active_pane()` = maximized-or-focused), each pane's top border has an `m▢`/`m▣` button (`max_click`, hit-tested + hover-wired), and `1`/`2`/`3`/`4` (`focus_or_maximize_pane`) swap which pane is maximized when one is, else move focus. Only the **repo page**'s maximize persists (round-tripped through the legacy `repo_page_maximized` bool; list/info/result maximize is session-only). The repo page is special in the render path — it returns early with its own border footer and no status bar; List/Info/Result maximize fill the main area but keep the status bar. `visible_panes` returns just the maximized pane. `is_pane_available` gates maximize (Info needs a selected repo, RepoPage needs an open page).

### Leader chords + header dropdowns

`app.pending_leader` (`View` = `v`, `Fold` = `z`) is a two-key chord: the first key arms it, the next picks. Handled in `main.rs` *before* the normal-key match. **Columns, sort, and status-filter are NOT leaders** — `t`/`s`/`f` open the header `[… ▾]` **dropdown** (`app.dropdown`, an `Option<Dropdown>` with `selected: Option<usize>`, anchored by the trigger's right edge so the menu right-aligns under it). The dropdown is the single columns/sort/filter UI: each row carries a mnemonic letter (the `app/dropdown.rs` const tables, `dropdown_activate_key`), pickable by mouse-hover+click, `↑↓`+Enter, or the letter; unavailable columns render dim + inert. Sort + filter rows use radio markers (`●`/`○`); columns use checkboxes (`[x]`/`[ ]`). Rendered by `render::modals::render_dropdown`; hover via the dropdown branch in `apply_hover`. Current top-level keymap (see README for the full table): `t` columns dropdown · `s` sort dropdown · `f` status-filter dropdown · `v` view (`vg` groups · `vt` tree) · `z` fold (`za`/`zo`/`zc`/`zO`/`zM`/`zR`) · `-`/`+`/`*` fold-all/expand-all/expand-subtree · `←`/`→` collapse-or-jump / expand · `/` name-filter · `r`/`R` retry · `e`/`E` refetch · `c` claude · `l` lazygit · `1`/`2`/`3`/`4` focus-or-maximize pane · `m` maximize/restore active pane · `b` favorite · `B` favorites-first. `Z` (shift) re-resolves dynamic groups.
