...
`app.pending_leader` (`Filter` = `f`, `View` = `v`, `Fold` = `z`) is a two-key chord: the first key arms it, the next picks. Handled in `main.rs` *before* the normal-key match. **Columns + sort are NOT leaders** — `t`/`s` open the header `[… ▾]` **dropdown** (`app.dropdown`, an `Option<Dropdown>` with `selected: Option<usize>`, anchored by the trigger's right edge so the menu right-aligns under it). The dropdown is the single columns/sort UI: each row carries a mnemonic letter (the `app/dropdown.rs` const tables, `dropdown_activate_key`), pickable by mouse-hover+click, `↑↓`+Enter, or the letter; unavailable columns render dim + inert. Rendered by `render::modals::render_dropdown`; hover via the dropdown branch in `apply_hover`. Current top-level keymap (see README for the full table): `t` columns dropdown · `s` sort dropdown · `f` status-filter · `v` view (`vg` groups · `vt` tree) · `z` fold (`za`/`zo`/`zc`/`zO`/`zM`/`zR`) · `-`/`+`/`*` fold-all/expand-all/expand-subtree · `←`/`→` collapse-or-jump / expand · `/` name-filter · `r`/`R` retry · `e`/`E` refetch · `c` claude · `l` lazygit · `1`/`2`/`3`/`4` focus-or-maximize pane · `m` maximize/restore active pane · `b` favorite · `B` favorites-first. `Z` (shift) re-resolves dynamic groups.
