...
- **`plain.rs`** — the `--no-tui` path. Output is **byte-compatible with the original bash `polygit-repos` script for a single-level scan** (`--depth 1` / a flat dir); a recursive scan additionally lists nested repos by their relative path. Grouping and the tree are TUI-only; plain mode does **not** do throttle adaptation/auto-retry (it's a one-shot batch).
