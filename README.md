...
- **Status-bar hints** — the whole hint ("s sort", "f by-status", "/ filter", …), not just the key. While a leader menu is armed (e.g. `f`/`s`/`t`) the rest of the footer dims and goes inert, and the armed leader's own trigger gets a highlight pill so it's clear which menu is open. The active tags sit next to their hints and are clickable too: `⟪name ▲⟫` flips the sort direction, `[needle]` clears the name filter, `{failed}` resets the status filter. In "[ ] resize", `[` and `]` nudge the split directly. The right side shows a clickable `vX.Y.Z` tag (opens the **Changelog**), a clickable `built … ago` tag (opens the Build info modal), and clickable `, settings · ? help · q quit`.
