...
- **Merged/gone-upstream → suggested "switch to base (& delete)"**: when a repo's current branch has a *gone* upstream (it was merged and its remote branch deleted), polygit suggests a way back. It computes a **deduped candidate list** — the branch it was cut from (auto-detected fork parent), the repo's default branch (always included), refined with the **merged PR's target branch** when known (the most authoritative). The suggestion is **dynamic**: when the dead branch is **fully merged** into the base it becomes `git switch <base> && git pull && git branch -d <branch>` ("Switch to `<base>` & delete `<branch>`"), cleaning up the local branch too; when it isn't (e.g. squash-merged) it stays `git switch <base> && git pull`. The delete uses the safe `git branch -d` (never `-D`), so an unmerged branch is never lost. It shows in **four places**: the **info panel** (a `SUGGESTED` section with an `Actions` row, one clickable action per candidate + a `Copy command ⧉` whose dwell tooltip reveals the full command), the **result/log panel** (a `⎇ switch to <base> …` chip), the **repo page** (a `suggest` line + an `S switch …` footer hint), and the **kebab (`⋮`)** (a run-item per candidate + a *Copy switch command* item). Pressing `S` (or clicking any run affordance) switches, pulls, and (when merged) deletes in the background; the suggestion self-clears once the new branch's live upstream is picked up
...
- Footer command hints **dim and go inert when not applicable** (never hidden) — e.g. info/diff/page/claude/lazygit/open/copy when no repo is selected, the fold + group/tree hints when there's nothing to fold. While a **modal is open** the background recedes — all pane borders + titles dim — and the whole footer goes inert except `, settings` · `? help` · `^L repaint` · `q` (which stay clickable **and hover-highlight** from inside any modal), with `q` reading **`close`** (it closes the modal instead of quitting). Only **one modal is ever open at a time** — clicking `, settings` while help is up closes help and opens settings (no stacking)
...
- **File explorer** (`Ctrl-E`, or the `Explore files…` kebab item): a two-pane browser rooted at the selected repo — a directory **list pane** beside a **syntax-highlighted preview pane** (powered by `two-face`/`syntect`, high-contrast themes that track the app's dark/light), split by a **draggable divider** (`[`/`]` or drag, highlights on hover).
  ...
  - **`.gitignore`-aware** — node_modules / dist / build output / `.git` are hidden by default (keeping the listing and folder-size walk fast); **`i`** toggles them. **Fuzzy file finder** (`/`) filters as you type and **live-previews** the focused match; in the tree it searches **recursively**, auto-expanding matches. The **preview is virtualized** (only the visible window is highlighted, so huge files open instantly) with a **horizontal scrollbar** for long lines; **`d`** toggles relative vs absolute date stamps; **`Backspace`** goes up a directory.
  - **`p` pins ⇄ floats** the explorer — floating renders it as a **draggable, resizable window** instead of the docked centered panel: drag the title bar to move it, drag the bottom-right `◢` grip to resize, or `Alt`+arrows to nudge / `Alt+Shift`+arrows to resize by keyboard. The mode persists across re-opens and restarts.
...
- **`Ctrl+L` repaint**: force a full terminal clear + redraw when a terminal emulator's own rendering glitches and leaves stale glyphs on screen — the classic vim/tmux/htop/less "Ctrl+L" behavior. Also the clickable `[^L repaint]` footer chip; both work from any view or modal, including while another modal is open
...
| `d` | Cycle the Result pane's **Diff tab sub-display**: **log → raw → unified → split** (or click the Diff tab again). `log` = the pull/command output; `raw` = git's colored diff; `unified`/`split` = syntax-highlighted, GitHub-PR-style. Diff = working-tree changes, else the last pull's diff (capped + control-byte-sanitized so a huge/binary pull can't garble or hang the pane). Switches to the Diff tab first if another tab is active. The choice persists |
| `D` | Cycle the Result pane's **category tab**: **diff / tags / branches / commits / files** (or click a tab). Tabs render at the **top of the pane body** (mirroring the repo page's tab style, icon + item count) and only appear when they have something to show. **`tags`**/**`branches`** list the whole repo (mirrors the repo page's Branches tab); **`commits`**/**`files`** are what the last pull delivered (count badge) — commits as expandable blocks (clickable sha → GitHub, subject, one-line body preview, `rel-date · author` with the author linking to GitHub — resolved lazily via `gh api` + cached, since a plain commit email can't be turned into a profile link directly), files as a status-colored list. The choice persists |
...
| `S` | **Switch a merged/gone-upstream branch to base (& delete)** — only when the current branch's upstream is *gone* (it was merged and its remote deleted). Switches to the top suggested base (the branch it was cut from / the merged PR's target / the repo default), pulls, and — when the dead branch is fully merged — safely `git branch -d`s it, in the background. The full deduped candidate list is in the kebab (`⋮`); the info + result panels show the suggestion too |
| `Ctrl-E` | Open the **file explorer** for the selected repo (also the `Explore files…` kebab item): a two-pane browser — a directory **list** (name + toggleable `1`-`5` size/permissions/modified/created/kind columns, off by default) beside a **syntax-highlighted preview**, split by a draggable divider (`[`/`]`). `↑↓`/`jk` move · `←`/`h` up a dir · `⏎`/`→` open dir or focus preview · `tab` swap panes · `p` pin ⇄ float (drag the title bar / `◢` grip, or `Alt`+arrows) · `esc`/`q` close |
...
| `Ctrl+D` | Open the [documentation website](https://steven-pribilinskiy.github.io/polygit/) in the browser (moved from plain `D`, which now cycles the Result pane's category tab) |
...
- **Parallel pulls** (Workers): the max concurrent-pull cap, expressed as an **exact worker count** or a **percentage of your CPU cores** (a button group), with the value picked from a **dropdown**. Exact mode offers a sensible ladder for your machine — `1, 2, 4, 6, 8, 12, 16, 24, 32, …` (fine steps at the low end, growing to a max stride of 8, ending on your core count); percentage mode offers `25 / 50 / 75 / 100 %` (each labeled with the resolved worker count, e.g. `50% (16)`). Changes apply **live** — the shared concurrency gate resizes and in-flight pulls ramp up or down within about a second, no restart. `-j` / `PULL_JOBS` still overrides at launch (and shows as an exact pick). Default: 100 % of cores.
