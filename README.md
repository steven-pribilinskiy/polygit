...
- **Merged/gone-upstream → suggested "switch to base & pull"**: when a repo's current branch has a *gone* upstream (it was merged and its remote branch deleted), polygit suggests a way back. It computes a **deduped candidate list** — the branch it was cut from (auto-detected fork parent), the repo's default branch (always included), refined with the **merged PR's target branch** when known (the most authoritative) — and offers `git switch <base> && git pull` in **four places**: the **info panel** (a `SUGGESTED` section, one clickable command per candidate, each with a `⧉` copy), the **result/log panel** (a `⎇ switch to <base> & pull` chip), the **repo page** (a `suggest` line + an `S switch & pull` footer hint), and the **kebab (`⋮`)** (a run-item per candidate + a *Copy switch command* item). Pressing `S` (or clicking any run affordance) switches to the top candidate and pulls in the background; the suggestion self-clears once the new branch's live upstream is picked up
...
| `S` | **Switch a merged/gone-upstream branch to base & pull** — only when the current branch's upstream is *gone* (it was merged and its remote deleted). Switches to the top suggested base (the branch it was cut from / the merged PR's target / the repo default) and pulls, in the background. The full deduped candidate list is in the kebab (`⋮`); the info + result panels show the suggestion too |
