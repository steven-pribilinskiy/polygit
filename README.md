...
- <a name="pr-viewer"></a>**PR viewer modal** — click any PR link (info-panel **Pull Request** value, repo-page HEAD `#N`, or a list **PRs** cell) to read the full pull request without leaving the terminal: title, state, base ← head, author, date, `+adds −dels`, labels, the description, and **every review and comment**, all rendered as markdown (headings, bullets, blockquotes, rules, links). `j`/`k`·`↑`/`↓`/`g`/`G`/`PgUp`/`PgDn` + wheel + draggable scrollbar scroll it; `o` opens it on GitHub; `esc`/`q`/`[x]`/outside-click closes
