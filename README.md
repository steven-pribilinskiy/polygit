...
- <a name="pr-viewer"></a>**PR viewer modal** — click any PR link (info-panel **Pull Request** value, repo-page HEAD `#N`, or a list **PRs** cell) to read the full pull request without leaving the terminal. A meta header (state, `head → base`, author, date, `+adds −dels`, labels) sits above the **description** and **every review/comment**, each as a **collapsible section** (click a `▾`/`▸` header; `z` or a `[− collapse all]`/`[+ expand all]` control folds them all when there's >1 comment). Bodies render as markdown (headings, bullets, blockquotes, rules, links; raw HTML stripped). **`/` searches** across description + comments (filters to matches, highlights hits). A loading skeleton shows while `gh pr view` runs. `j`/`k`·`↑`/`↓`/`g`/`G`/`PgUp`/`PgDn` + wheel + draggable scrollbar scroll; `o` opens it on GitHub; `esc`/`q`/`[x]`/outside-click closes
