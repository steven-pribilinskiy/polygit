...
| `d` | Cycle the result view: **log → commits → files → raw → unified → split** (or click a chip in the pane's footer switcher). `log` = the pull/command output; **`commits`** = the commits the pull delivered (count badge), each an expandable block — clickable sha → GitHub, subject, one-line body preview that expands/collapses on click, `rel-date · author` with the author linking to GitHub (mirrors the info panel's "Last commit"); **`files`** = the files the pull changed (count badge), a status-colored list; `raw` = git's colored diff; `unified`/`split` = syntax-highlighted, GitHub-PR-style. Diff = working-tree changes, else the last pull's diff (capped + control-byte-sanitized so a huge/binary pull can't garble or hang the pane). The choice persists |
